...
void Usuario::aparcaCoche(Coche *coche, PuntoRecarga *destination, PuntoRecarga *destinoEsperado) {
    if (!cochealquilado) {
    ...
    if (!destination || !destinoEsperado) {
        std::cerr << "Error: El punto de recarga destino no es valido. " << std::endl;
        return;
    }

    if (destination->getId() != destinoEsperado->getId()) {
        UTM posicionActual = destinoEsperado->getPosicion();
        UTM posicionDestino = destination->getPosicion();

        float distanciaKm = linkReanelcar_->getMallaRegular().distanciaHaversine(posicionActual.getLat(),
                                                                                 posicionActual.getLon(),
                                                                                 posicionDestino.getLat(),
                                                                                 posicionDestino.getLon());
        decrementarPuntosPorDistancia(distanciaKm);
    }

    if (!linkReanelcar_->colocarCochePR(coche, destination)) {
        std::cerr << " El usuario: " << this->getNombre() << " no pudo aparcar el coche. " << std::endl;
    }

    this->setCochealquilado(nullptr);
}
...
void Usuario::decrementarPuntosPorDistancia(float distanciaKm) {
    int puntosPerdidos = static_cast<int>(std::ceil(distanciaKm)) * 2;
    puntos -= puntosPerdidos;
}
