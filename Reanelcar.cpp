...
std::vector<std::pair<Usuario, Trayecto>> Reanelcar::trayectosEnFecha(Fecha f) {
    ...
    for (Usuario &usuario: usuarios) {
        for (std::pair<const Fecha, Trayecto *> &parTrayecto: usuario.getTrayectos()) {
...
void Reanelcar::eliminarUsuariosPorNombre(std::string &prefijo) {
    ...
    for (std::list<Usuario>::iterator it = usuarios.begin(); it != usuarios.end();) {
...
void Reanelcar::rellenaMalla() {
    for (auto &[matricula, coche]: coches) {
        if (coche.getPuntoRecarga() != nullptr) {
            UTM pos = coche.getPuntoRecarga()->getPosicion();
            mallaRegular.insertar(pos.getLat(), pos.getLon(), reinterpret_cast<Coche *&>(coche));
        }
    }
}

std::vector<Coche *> Reanelcar::buscarCochesRadio(UTM pos, float radiokm) {
    std::vector<Coche *> cochesEnRadio;
    float radioM = radiokm * 1000;

    std::vector<Coche *> encontrados = mallaRegular.buscarRadio(pos.getLat(), pos.getLon(), radioM);

    cochesEnRadio.reserve(encontrados.size());

    for (Coche *&coche: encontrados) {
        cochesEnRadio.push_back(coche);
    }

    return cochesEnRadio;
}

Coche *Reanelcar::buscarCocheMasCercano(UTM pos) {
    std::vector<Coche *> cochesEnRadio = mallaRegular.buscarRadio(pos.getLat(), pos.getLon(), INT_MAX);

    Coche *cocheMasCercano = nullptr;
    float distanciaMinima = std::numeric_limits<float>::max();

    for (Coche *coche: cochesEnRadio) {
        if (coche->getPuntoRecarga() != nullptr) {
            float distancia = mallaRegular.distanciaHaversine(pos.getLat(), pos.getLon(),
                                                              coche->getPuntoRecarga()->getPosicion().getLat(),
                                                              coche->getPuntoRecarga()->getPosicion().getLon());
...
MallaRegular<Coche *> &Reanelcar::getMallaRegular() {
    return mallaRegular;
}

void Reanelcar::setMallaRegular(MallaRegular<Coche *> &mallaRegular) {
    Reanelcar::mallaRegular = mallaRegular;
}