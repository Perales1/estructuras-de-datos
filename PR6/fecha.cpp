...
using namespace std; //Por comodidad, abrimos en el módulo el espacio de nombres std
...
bool Fecha::operator<(const Fecha &f) const { // Modificado porque no funcionaba
    if (anio != f.anio)
        return anio < f.anio;
    if (mes != f.mes)
        return mes < f.mes;
