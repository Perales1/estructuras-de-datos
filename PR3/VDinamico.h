 ...
 */
#ifndef PR3AVL_VDINAMICO_H
#define PR3AVL_VDINAMICO_H
...
class VDinamico {
    ...
    unsigned int fisico;          ///< Tamaño físico del vector (capacidad).
...
public:
     ...
     */
    VDinamico<T> &operator=(VDinamico<Coche *> *other);
...
template<class T>
VDinamico<T>::VDinamico():fisico(1), tamlogico(0) {
    mem = new T[fisico];
}

template<class T>
VDinamico<T>::VDinamico(unsigned int _tamlogico):tamlogico(_tamlogico) {
    fisico = std::pow(2, std::ceil(std::log2(_tamlogico + 1)));
    mem = new T[fisico];
}

template<class T>
VDinamico<T>::VDinamico(const VDinamico<T> &other): tamlogico(other.tamlogico), fisico(other.fisico) {
    mem = new T[fisico];
...
VDinamico<T>::VDinamico(const VDinamico<T> &other, unsigned int posicioninicial, unsigned int numelementos) {
    ...
    fisico = std::pow(2, std::ceil(std::log2(numelementos + 1)));
    mem = new T[fisico];
...
template<class T>
VDinamico<T> &VDinamico<T>::operator=(VDinamico<Coche *> *other) {
    if (this != other) {
        delete[] mem;
...
T VDinamico<T>::borrar(unsigned int pos) {
    ...
    if (tamlogico < fisico / 3 && fisico > 1) {
        fisico /= 2;
        T *_mem = new T[fisico];
        std::copy(mem, mem + tamlogico, _mem);
        delete[] mem;
